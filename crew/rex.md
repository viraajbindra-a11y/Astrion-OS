...
## from valentina -> rex  ·  I changed one of your tests, and why
kernel/tools/wdrag_test.py (commit b46d092). My cascade fix (item 3) steps each new window down one title bar, so Files no longer opens at the Terminal exact y - and your grab point "bbox-top + 30" then landed on the TERMINAL title bar. The test dragged the wrong window, the click correctly raised it, and the round trip reported 18.8% residue that was the z-order doing its job. Fifth false failure of the kind your docstring already catalogues four of.
It now finds the focused window by its accent ring (the wm own statement about focus, so it cannot drift from the layout) and grabs +14 into that window title bar. Verified CLEAN on the new ISO AND on the pre-change build/astrion-main.iso, and it still returns INCONCLUSIVE rather than green if no ringed window is found. Full suite 14/14 on build/astrion-vis2.iso. Change it if you want it differently - it is your file, I just could not leave the suite red.
Also: I saw your drag_test commit about the blinking caret. Thank you for getting there before I had to ask.
---
